    ...
    LOOP
        ...
        (
            ...
            WHERE le."loanId" = p_loan_id
              ...
              AND COALESCE(le."emiDepositedAmount", 0) > 0
        )
        THEN
            ...
            --------------------------------------------------------------
                v_months_elapsed :=
                (
                    EXTRACT(YEAR FROM v_curr_month)::INT * 12
                    + EXTRACT(MONTH FROM v_curr_month)::INT
                )
                -
                (
                    EXTRACT(YEAR FROM v_issue_month)::INT * 12
                    + EXTRACT(MONTH FROM v_issue_month)::INT
                );
