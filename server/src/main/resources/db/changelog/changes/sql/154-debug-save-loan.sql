...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO debug_function_log(step, variable, value)
    VALUES
    ('START', 'loanId', v_loan_id::text),
    ('START', 'v_amt', v_amt::text),
    ('START', 'v_prepay', v_prepay::text),
    ('START', 'v_old_monthly_int', v_old_monthly_int::text);
...
v_months_elapsed :=
    (
        EXTRACT(YEAR FROM v_emi_month_dt)::int * 12
        + EXTRACT(MONTH FROM v_emi_month_dt)::int
    )
    -
    (
        EXTRACT(YEAR FROM v_issue_date)::int * 12
        + EXTRACT(MONTH FROM v_issue_date)::int
    );

    INSERT INTO debug_function_log(step, variable, value)
    VALUES
    ('MONTHS ELAPSED', 'v_months_elapsed', v_months_elapsed::text);
    ...
    IF COALESCE(v_prepay, 0) > 0 THEN
      ...
      INSERT INTO debug_function_log(step, variable, value)
      VALUES
      ('REMAINING AFTER CURRENT', 'v_remaining_after_cur', v_remaining_after_cur::text);
      ...
      INSERT INTO debug_function_log(step, variable, value)
      VALUES
      ('UNPAID REMAINING', 'v_paid_future', v_paid_future::text),
      ('UNPAID REMAINING', 'v_unpaid_remaining', v_unpaid_remaining::text);
      ...
      THEN
          ...
          v_revised_rate_months :=
              ...
              INSERT INTO debug_function_log(step, variable, value)
              VALUES
              ('RATE SPLIT', 'v_old_rate_months', v_old_rate_months::text),
              ('RATE SPLIT', 'v_revised_rate_months', v_revised_rate_months::text);
      ...
      INSERT INTO debug_function_log(step, variable, value)
      VALUES
      ('INTEREST SAVED', 'v_interest_saved', v_interest_saved::text);

           INSERT INTO debug_function_log(step, variable, value)
           VALUES
           ('BEFORE DEDUCTION', 'loan.totalInterest', v_ld."totalInterest"::text),
           ('BEFORE DEDUCTION', 'loan.totalAmount', v_ld."totalAmount"::text),
           ('BEFORE DEDUCTION', 'fund.expectedMaturity', v_totalExpMaturity::text);
      ...
      IF v_interest_saved > 0 THEN
        ...
        INSERT INTO debug_function_log(step, variable, value)
        VALUES
        ('AFTER DEDUCTION', 'expectedMaturity', v_totalExpMaturity::text);
      END IF;
    END IF;
   ...
   IF v_isClosed THEN
       ...
       INSERT INTO debug_function_log(step, variable, value)
       VALUES
       ('PRINCIPAL ONLY', 'v_amt', v_amt::text),
       ('PRINCIPAL ONLY', 'v_prepay', v_prepay::text);

       INSERT INTO debug_function_log(step, variable, value)
       VALUES
       ('PRINCIPAL ONLY CHECK', 'v_amt', COALESCE(v_amt,0)::text),
       ('PRINCIPAL ONLY CHECK', 'v_prepay', COALESCE(v_prepay,0)::text);
       IF COALESCE(v_amt,0) = 0
          AND COALESCE(v_prepay,0) > 0 THEN

            INSERT INTO debug_function_log(step, variable, value)
            VALUES
            ('PRINCIPAL ONLY', 'ENTERED', 'TRUE');
