...
& $gradlew `
    ...
    :lambda:email-processor:clean `
    :lambda:email-processor:build `
    --no-daemon `
    --console=plain
...
$artifactSrc = "$projectRoot\lambda\email-processor\build\libs\email-processor.jar"
$artifactDstDir = "$projectRoot\infra\artifacts"
$artifactDst = "$artifactDstDir\email-processor.jar"
