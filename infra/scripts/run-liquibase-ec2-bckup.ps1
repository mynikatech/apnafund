...
$DefaultsFile = switch ($Env) {
    'prod' { Join-Path $repoRoot "server\src\main\resources\db\liquibase.ec2-prod.properties" }
    'test' { Join-Path $repoRoot "server\src\main\resources\db\liquibase.ec2-test.properties" }
    default { Join-Path $repoRoot "server\src\main\resources\db\liquibase.ec2-dev.properties" }
}
# sanity check
if (-not (Test-Path $DefaultsFile)) {
    Write-Host "Defaults file not found at: $DefaultsFile" -ForegroundColor Red
    exit 8
}
